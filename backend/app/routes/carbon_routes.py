...
def calculate_carbon():
    ...
    if len(_keys) >= 2:
        ...
        if emission_factor is not None:
            is_valid_f, factor_val = validate_numeric(
                emission_factor, "emission_factor", min_value=0.0
            )
            if not is_valid_f:
    ...
    if not is_valid_kwh:
        return jsonify({"status": "error", "message": kwh_val}), 400

    factor_val = None
    if emission_factor is not None:
        is_valid_factor, factor_val = validate_numeric(
            emission_factor, "emission_factor", min_value=0.0
        )
