...
def calculate_energy():
    ...
    if "devices" in data:
        ...
        for i, dev in enumerate(devices):
            ...
            # Validasi watt > 0
            is_valid_watt, watt_val = validate_numeric(
                watt, f"watt perangkat ke-{i + 1} ({name})", min_value=0.01
            )
            if not is_valid_watt:
                return jsonify({"status": "error", "message": watt_val}), 400

            # Validasi jam > 0 dan <= 24
            is_valid_hours, hours_val = validate_numeric(
                hours_per_day,
                f"jam penggunaan perangkat ke-{i + 1} ({name})",
                min_value=0.01,
                max_value=24.0,
            )
            ...
            validated_devices.append(
                {"device_name": html.escape(name), "watt": watt_val, "hours_per_day": hours_val}
            )
        ...
        insights_data = InsightService.generate_insights(
            result["devices"], result["total_daily_kwh"], result["total_monthly_kwh"]
        )
    ...
    # Validasi jam > 0 dan <= 24
    is_valid_hours, hours_val = validate_numeric(
        hours_per_day, "hours_per_day", min_value=0.01, max_value=24.0
    )
    if not is_valid_hours:
        return jsonify({"status": "error", "message": hours_val}), 400
    ...
    devices_list = [result]
    insights_data = InsightService.generate_insights(
        devices_list, result["daily_kwh"], result["monthly_kwh"]
    )
    ...
    if save:
        try:
            ...
            cost_data = CostService.calculate_all(
                result["daily_kwh"], result["monthly_kwh"], result["yearly_kwh"]
            )
            carbon_data = CarbonService.calculate_all(
                result["daily_kwh"], result["monthly_kwh"], result["yearly_kwh"]
            )
            hist = save_simulation(
    ...
    cost_inline = CostService.calculate_all(
        result["daily_kwh"], result["monthly_kwh"], result["yearly_kwh"]
    )
    carbon_inline = CarbonService.calculate_all(
        result["daily_kwh"], result["monthly_kwh"], result["yearly_kwh"]
    )
